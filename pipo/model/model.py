...
if __name__ == "__main__":
    ...
    print(
        f"📝 Layer execution order: {config.layer_names[:3]}...{config.layer_names[-2:]}"
    )
                ...
                if layer_idx == 0:
                    ...
                    for name, param in state_dict.items():
                        set_module_tensor_to_device(
                            embed_layer, name.split(".")[-1], "cpu", value=param
                        )
                    ...
                    attn_weights = [
                        ...
                        if "self_attn" in k and "weight" in k
                    ]
                    if attn_weights:
                        q_proj_weight = state_dict.get(
                        ...
                        if q_proj_weight is not None:
                            ...
                            hidden_states = torch.randn(
                                1, 10, hidden_size, dtype=model_dtype
                            )
                            print(
                                f"  Test input: {hidden_states.shape} ({hidden_states.dtype})"
                            )
                    ...
                    if norm_weight is not None:
                        ...
                        with torch.no_grad():
                            ...
                            print(
                                f"  Normalized output: {output.shape} ({output.dtype})"
                            )
                    ...
                    if lm_weight is not None:
                        ...
                        print(
                            "  ⚠️ No lm_head.weight found - might be tied to embedding"
                        )
